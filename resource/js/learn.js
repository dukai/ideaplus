...
(function($) {
    ...
    $.fn.cycle.defaults = {fx: 'fade',timeout: 6000,timeoutFn: null,continuous: 0,speed: 1000,speedIn: null,speedOut: null,next: null,prev: null,prevNextClick: null,prevNextEvent: 'click',pager: null,pagerClick: null,pagerEvent: 'click',pagerAnchorBuilder: null,before: null,after: null,end: null,easing: null,easeIn: null,easeOut: null,shuffle: null,animIn: null,animOut: null,cssBefore: null,cssAfter: null,fxFn: null,height: 'auto',startingSlide: 0,sync: 1,random: 0,fit: 1,containerResize: 1,pause: 0,pauseOnPagerHover: 0,autostop: 0,autostopCount: 0,delay: 0,slideExpr: null,cleartype: !$.support.opacity,cleartypeNoBg: false,nowrap: 0,fastOnEvent: 0,randomizeEffects: 1,rev: 0,manualTrump: true,requeueOnImageNotLoaded: true,requeueTimeout: 250};
})(jQuery);
(function($) {
...
function loading() {
    $('#logo').stop().delay(150).fadeOut(150, function() {
        $('#ball').stop().delay(150).animate({width: 0,height: 0,marginTop: 0,marginLeft: 0,}, {duration: 250,easing: 'easeInOutExpo',complete: function() {
                if (url != '#/') {
                    var lUrl = url.split("/");
                    if (lUrl[1] == 'line')
                        cscrf = true;
                            ...
                            $('#home_nav_2').fadeIn(100, function() {
                                $('#home_nav_3').fadeIn(100, function() {
                                    ...
                                    $('#home_text').find('img').each(function() {
                                        ...
                                        $(this).css({'height': (obj.h / scale) + 'px','width': (obj.w / scale) + 'px','margin-top': (obj.mT / scale) + 'px','margin-left': (obj.mL / scale) + 'px',});
                                        counter++;
                                    });
                                    var counter = 0;
                                    ...
                                    $('#mission_soluzioni_text').find('img').each(function() {
                                        var obj = missiononSoluzioniArray[counter];
                                        $(this).css({'height': (obj.h / scale) + 'px','width': (obj.w / scale) + 'px','margin-top': (obj.mT / scale) + 'px','margin-left': (obj.mL / scale) + 'px',});
                                        counter++;
                                    });
                                    scale = 1;
                                    ...
                                    $('#home_text').find('img').each(function() {
                                        var obj =  [counter];
                                        $(this).delay(timeOut * (1 + counter)).animate({opacity: 1,height: (obj.h / scale) + 'px',width: (obj.w / scale) + 'px',marginTop: (obj.mT / scale) + 'px',marginLeft: (obj.mL / scale) + 'px',}, {duration: timeDuration,easing: 'easeInOutExpo'});
                                        counter++;
                                    });
                                });
                            });
...
(function($) {
    ...
    var createPreloadContainer = function() {
        qLimageContainer = $("<div></div>").appendTo("body").css({display: "none",width: 0,height: 0,overflow: "hidden"});
        for (var i = 0; qLimages.length > i; i++) {
            $.ajax({url: qLimages[i],type: 'HEAD',success: function(data) {
                    qLimageCounter++;
    ...
    $.fn.queryLoader2 = function() {
        ...
        $('.glare').fadeIn(500, function() {
            $('#ball').stop().delay(250).animate({width: '240px',height: '240px',marginTop: '-' + 120 + 'px',marginLeft: '-' + 120 + 'px',}, {duration: 250,easing: 'easeInOutExpo',complete: function() {
...
function portfolioClose() {
    if ($('body').css('overflow') == 'auto' || !queryAjaxClose)
        return false;
    queryAjaxClose = false;
    $('body').css('overflow-y', 'scroll');
    $('#portfolio_box').css('overflow-y', 'hidden');
    location.hash = '/';
    $('#portfolio_box').animate({marginTop: '-' + $(window).height() + 'px'}, {queue: false,duration: 1000,easing: 'easeInOutExpo',complete: function() {
            $('body').css('overflow', 'auto');
            slide = [];
            $('#portfolio_box').hide();
...
function portfolio(x) {
    ...
    var xTitle = h[0];
    $('#portfolio_box').show();
    $('#portfolio_box').animate({marginTop: 0}, {queue: false,duration: 1000,easing: 'easeInOutExpo',complete: function() {
            ...
            $.ajax({beforeSend: function() {
                ...
                },type: "post",url: 'portfolio.php',data: {id: xId},async: true,success: function(text) {
                    $(text).hide().appendTo('#portfolio_box .glare');
                    $('#portfolio_slide').css({'height': $(window).height() - 140 + 'px'});
                    ...
                    $('#portfolio_slide .slide').css({'height': $(window).height() - 140 + 'px'});
                            ...
                            $('#portfolio_slide').fadeIn(250, function() {
                                ...
                                if ($('#portfolio_prev').css('background') != undefined) {
                                    $(document).keydown(function(e) {
                                        ...
                                        if (e.which == 37 && $('#portfolio_prev_t').css('z-index') == undefined)
                                            $('#portfolio_slide').cycle('prev');
                                        if (e.which == 39 && $('#portfolio_prev_t').css('z-index') == undefined)
                                            $('#portfolio_slide').cycle('next');
                                    });
                                }
                            })
                        })
                    ...
                    if ($('#portfolio_prev').css('background') != undefined) {
                        $('#portfolio_slide').cycle({fx: 'scrollHorz',easing: 'easeInOutExpo',timeout: 0,speed: 750,next: '#portfolio_next, #portfolio_slide img',prev: '#portfolio_prev',prevNextClick: function(isNext, zeroBasedSlideIndex, slideElement) {
...
$.fn.parallasseHome = function(y) {
    ...
    return this.each(function() {
        $(window).bind('scroll', function() {
            ...
            if ((scrollTop <= height)) {
                element.css({"margin-top": (marginTop - (scrollTop * y)) + "px"});
            }
        });
    });
};
$.fn.parallasseContatti = function(y) {
    ...
    return this.each(function() {
        $(window).bind('scroll', function() {
            ...
            if ((scrollTop > height)) {
                element.css({"margin-top": "-" + ((Math.floor($(window).height() * y) - Math.floor((scrollTop - height) * y))) + "px"});
            } 
            else {
                element.css({"margin-top": "-" + Math.floor($(window).height() * y) + "px"});
            }
        });
    });
};
function resizeContatti() {
    if ($(window).height() < 920) {
        $('#contatti_text').css({'top': '460px',});
        $('#map').css({'height': ($(window).height() - 300) + 'px',});
        if ($('#map').height() <= 280) {
            $('#map').css({'height': 280 + 'px',});
        }
    } 
    else {
        $('#contatti_text').css({'top': '50%',});
        $('#map').css({'height': 620 + 'px',});
    }
}
$(window).scroll(function() {
    ...
    if (!missionPensieroCheck && !missionGiocoCheck && !missionSoluzioniCheck && (scrollTop >= (portfolio - 100))) {
        $('#mission img').stop().animate({opacity: op}, 100);
    }
    ...
    if (!loadAnCheck && !debug) {
        if (missionPensieroCheck && scrollTop >= (missionPensiero - windowHeight / 2)) {
            ...
            $('#mission_pensiero_text').find('img').each(function() {
                var obj = missionPensieroArray[counter];
                $(this).delay(timeOut * (1 + counter)).animate({opacity: 1,height: obj.h + 'px',width: obj.w + 'px',marginTop: obj.mT + 'px',marginLeft: obj.mL + 'px',}, {duration: timeDuration,easing: 'easeInOutExpo'});
                counter++;
            });
            setTimeout(function() {
        ...
        if (!missionPensieroCheck && scrollTop >= (missionPensiero - windowHeight / 2) && scrollTop <= (missionGioco + windowHeight / 2)) {
            $('#mission_pensiero_text').find('img').each(function() {
                if (scrollTop <= ($(this).offset().top - 80)) {
                    ...
                    $(this).stop().animate({opacity: op}, 100);
                }
            });
        }
    ...
    $('a[href*=#]').click(function() {
        if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
            ...
            if ($target.length) {
                ...
                $(scrollCheck).animate({scrollTop: targetOffset,}, {duration: 1500,easing: 'easeInOutExpo',queue: false});
                return false;
            }
        }
    });
...
function layoutResize() {
    ...
    $('#portfolio_slide, #portfolio_slide .slide').css({'height': $(window).height() - 140 + 'px'});
    if ($(window).width() < 1120) {
        $('#layout > header').css({'left': '0','margin-left': '10px',});
        $('#loading, #home, #home_nav_1, #home_nav_2, #home_nav_3, #mission, #portfolio, #contatti').css({'background-position': 'left top',});
    } else {
        $('#layout > header').css({'left': '50%','margin-left': '-550px',});
        $('#loading, #home, #home_nav_1, #home_nav_2, #home_nav_3, #mission, #portfolio, #contatti').css({'background-position': 'center top',});
    }
    var scale = 1;
    if ($(window).height() < 800 && $(window).height() > 400) {
        scale = (800 / $(window).height());
        $('#home_text').css({'top': '50%',});
    } else if ($(window).height() <= 400) {
        scale = 2;
        $('#home_text').css({'top': '200px',});
    }
    var counter = 0;
    $('#home_text').find('img').each(function() {
        var obj = homeArray[counter];
...
function magnetEffect() {
    ...
    $(scrollCheck).animate({scrollTop: result,}, {duration: 1500,easing: 'easeInOutExpo',queue: false});
}
...
$(document).ready(function() {
    ...
    $(scrollCheck).stop().animate({scrollTop: 0}, {complete: function() {
            if (!debug) {
                $('img').attr('alt', '');
                $('body').css('overflow', 'hidden');
            ...
            if (!debug) {
                $('#home_text img' 
                + ', #mission_pensiero_text img' 
                + ', #mission_gioco_text img' 
                + ', #mission_soluzioni_text img').css('opacity', 0);
                $('.glare' 
                + ',#layout > header' 
                + ', #home' 
                + ', #home_nav_1' 
                + ', #home_nav_2' 
                + ', #home_nav_3').css('opacity', 0);
